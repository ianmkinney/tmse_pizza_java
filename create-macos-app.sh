#!/bin/bash
# Script to create macOS .app bundle using jpackage
# Requires: Java 21+, jpackage tool

echo "Creating macOS .app bundle for TMSE Pizza Application..."
echo ""

# Check if jpackage is available
if ! command -v jpackage &> /dev/null; then
    echo "ERROR: jpackage not found"
    echo "jpackage is included with Java 14+"
    echo "Make sure Java 21+ is installed and in PATH"
    exit 1
fi

# Check if JAR exists
if [ ! -f "TMSE_Pizza.jar" ]; then
    echo "ERROR: TMSE_Pizza.jar not found"
    echo "Please build the application first: ./build.sh"
    exit 1
fi

# Create app name and version
APP_NAME="TMSE Pizza"
APP_VERSION="1.0.0"
MAIN_CLASS="com.tmse.pizza.PizzaApp"

echo "Building macOS .app bundle..."
echo "Application: $APP_NAME"
echo "Version: $APP_VERSION"
echo ""

# Create .app bundle using jpackage
jpackage \
    --input . \
    --name "$APP_NAME" \
    --main-jar TMSE_Pizza.jar \
    --main-class "$MAIN_CLASS" \
    --type app-image \
    --app-version "$APP_VERSION" \
    --description "TMSE Pizza Ordering System" \
    --vendor "TMSE" \
    --copyright "2025 TMSE Pizza" \
    --mac-package-name "$APP_NAME" \
    --java-options "--module-path javafx-sdk-21/lib --add-modules javafx.controls,javafx.fxml" \
    --java-options "-Xmx512m" \
    --dest dist

if [ $? -eq 0 ]; then
    echo ""
    echo "SUCCESS: .app bundle created in dist/$APP_NAME.app"
    echo ""
    echo "To create a .dmg installer, run:"
    echo "  ./create-macos-dmg.sh"
else
    echo ""
    echo "ERROR: Failed to create .app bundle"
    echo ""
    echo "Note: jpackage requires Java 21+ with jpackage tool"
    exit 1
fi

